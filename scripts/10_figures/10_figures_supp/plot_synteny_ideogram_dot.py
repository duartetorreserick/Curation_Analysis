...
DOT_TOKENS = ['16', '25', '29', '30', '31', '32', '33', '34', '35', '36', '37']
...
def build_flip_set_from_centromeres_and_telomeres(seq_sizes, centromeres, p_starts):
    """
    Determines which chromosomes should be flipped so the end where the
    centromere is closer points toward the center of the butterfly plot.
    If no centromere is available, falls back to p-arm telomere position.
    """
    flip_set = set()
    for chrom, size in seq_sizes.items():
        ...
        tok_lookup = tok
        if tok == 'ZW':
            tok_lookup = 'W' if side == 'mat' else 'Z'
        if (tok_lookup, side) in centromeres:
            cs, ce = centromeres[(tok_lookup, side)]
            cen_mid = (cs + ce) / 2
            if cen_mid > size / 2:
                flip_set.add(chrom)
        elif chrom in p_starts:
...
def plot_butterfly_macro(
    ...
    # 1. Global X scale (restricted to dot chromosomes in tokens)
    max_global_size = 0
    for tok in tokens:
        for side in ('mat', 'pat'):
    ...
    SPACER = 250_000  # 250 kb spacer for dot chromosomes
    for tok in tokens:
        for side in ('mat', 'pat'):
            t2t_c = next((k for k in t2t_sizes.keys() if f'chromosome_{tok}' in k and (side == 'pat' if ('Pat' in k or tok == 'Z') else side == 'mat')), None)
            ...
            for asm_d in (single_data, dual_data, ont_data):
                ...
                for s in asm_d['sizes']:
                    if s.startswith(f"{stem}_unloc_") and (not suf or s.endswith(suf)):
                        u_set.add(s)
                for u in u_set:
                    tot += SPACER + asm_d['sizes'].get(u, 100_000)
                max_global_size = max(max_global_size, tot)

    max_global_size = max_global_size + 500_000
    ...
    for row_idx, tok in enumerate(tokens):
        ...
        ax_chr.text(0.70, y_mid_row, lbl_text, ha='center', va='center',
                    fontsize=14.0, fontweight='bold', color='#0F172A')
        ...
        for b_idx, (asm_title, asm, cov_sum_dict, c_dark, c_light, c_lbl) in enumerate(assemblies):
            ...
            for side, ax in [('mat', ax_mat), ('pat', ax_pat)]:
                ...
                def _unloc_sort_key(u):
                    ...
                    if has_nc:
                        tier = 0  # front: non-collinear ribbon
                    elif has_other:
                        tier = 1  # middle: other chains (recovered/collinear)
                    else:
                        tier = 2  # back: no chains
                    return (tier, u_num, u)

                unloc_qual = sorted(unloc_candidates, key=_unloc_sort_key)

                # Compute X-offsets for primary and unlinked scaffolds
                SPACER = 250_000  # 250 kb spacer between primary and unlinked
                offsets = {q_prim: 0}
                cur_x = q_prim_len
                for u in unloc_qual:
                    cur_x += SPACER
                    offsets[u] = cur_x
                    cur_x += asm['sizes'].get(u, 100_000)
                    ...
                    for gs, ge, gtype in asm['gaps'].get(q_prim, []):
                        ...
                        if 'CURATION' in gtype:
                            ax.plot([gx, gx], [y_asm - H_BAR_ASM / 2, y_asm + H_BAR_ASM / 2],
                                    color=COL_CUR_GAP, lw=0.7, zorder=9, solid_capstyle='butt')
                        else:
                            ax.plot([gx, gx], [y_asm - H_BAR_ASM / 2, y_asm + H_BAR_ASM / 2],
    ...
    ax_mat.set_title('Maternal', fontsize=18, fontweight='bold', pad=12, color='#1E293B')
    ax_pat.set_title('Paternal', fontsize=18, fontweight='bold', pad=12, color='#1E293B')
    ...
    # Extended Coordinate Axis with Ticks every 1 Mb at Bottom
    axis_max = int(np.ceil(max_global_size / 1_000_000)) * 1_000_000
    y_axis = 0.28
    tick_h = 0.10
    ...
    for ax, side in [(ax_mat, 'mat'), (ax_pat, 'pat')]:
        ...
        # Periodic ticks and labels every 1 Mb
        for tick_val in range(0, axis_max + 1, 1_000_000):
    ...
    legend_elements = [
        ...
        mpatches.Patch(facecolor=COL_RIB_REC, edgecolor=COL_RIB_REC_E, lw=0.4, alpha=ALPHA_RIB_REC, label='Uncovered Synteny'),
        mpatches.Patch(facecolor='none', edgecolor='#64748B', lw=0.9, ls='--', label='Unloc Scaffold'),
        mpatches.Patch(facecolor=COL_SWITCH, edgecolor='none', label='Switch Block'),
        plt.Line2D([0], [0], color=COL_CUR_GAP, lw=1.4, label='Curation Gap (Join)'),
        plt.Line2D([0], [0], color=COL_ASM_GAP, lw=0.9, label='Assembly Gap'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#000000', markeredgecolor='#000000',
                   markersize=7.5, label='Collinear Telomere'),
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='white', markeredgecolor='#000000',
                   markeredgewidth=1.2, markersize=7.5, label='Non-collinear Telo.')
    ]

    leg = ax_leg.legend(handles=legend_elements, loc='upper left',
                        bbox_to_anchor=(0.0, 0.0, 1.0, 1.0), mode='expand',
                        ncol=1, frameon=True,
                        facecolor='#F8FAFC', edgecolor='#CBD5E1', fontsize=10.5,
                        handlelength=1.4, handleheight=0.9, handletextpad=0.7,
                        borderpad=0.9, labelspacing=0.9)
